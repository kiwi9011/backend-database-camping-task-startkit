...
SELECT 
	...
	("CREDIT_PURCHASE".total - "USED_CREDIT".total) AS remaining_credit
FROM (SELECT user_id, SUM(purchased_credits) AS total
		FROM "CREDIT_PURCHASE"
		WHERE user_id = (SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io')
		GROUP BY user_id) AS "CREDIT_PURCHASE"
INNER JOIN (SELECT user_id, COUNT(*) AS total
		FROM "COURSE_BOOKING"
		WHERE user_id = (SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io')
		AND status != '課程已取消'
		GROUP BY user_id) AS "USED_CREDIT"
ON "USED_CREDIT".user_id = "CREDIT_PURCHASE".user_id;
